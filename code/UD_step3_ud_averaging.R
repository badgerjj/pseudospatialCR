...
### load ud maps
### ~~~~~~~~~~~~
load(file.path(dir, "processed_data","kde_data_full.RData"))
...
use_df <- mov %>% dplyr::select(tag_id, cluster) %>% 
  mutate(
    ud = foreach(i=1:N)%do%{
      tot_hrs <- diff(as.numeric(range(mov$mi_samp[[i]][[1]]$timestamp)))/(3600)
...
size_n<-apply(table(mov[c("tag_id","cluster")]), 2, sum)

nms <- paste0("cluster_ud_", 1:max(use_df$cluster))

for(i in 1:max(use_df$cluster, na.rm=T)){
  dtmp <- use_df %>% filter(cluster==i) %>% 
  ...
  assign(nms[i], dtmp)
  save(list=nms[i], file=paste0("processed_data/cluster_uds/",nms[i],".RData"))
  ...
  ggsave(p_use, file=paste0("processed_data/ud_plots/kde_cluster_", i, ".png"), width=6.5, height=4, dpi="retina")
}
